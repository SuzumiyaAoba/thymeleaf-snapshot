 ...
 *
 * <p>This extension handles the lifecycle of snapshot tests:</p>
 * <ul>
 *   <li>Initializes the Thymeleaf template engine based on {@link SnapshotConfig}</li>
 *   <li>Creates and injects {@link Snapshot} instances into test methods</li>
 *   <li>Manages snapshot file storage via {@link SnapshotManager}</li>
 * </ul>
 *
 * <p>The {@link ThymeleafRenderer} and {@link SnapshotManager} are cached at the
 * class level to avoid re-creating heavyweight objects for each test method.</p>
 *
 * <h2>Usage</h2>
 * <pre>{@code
...
public class ThymeleafSnapshotExtension implements BeforeEachCallback, ParameterResolver {
    ...
    /**
     * System property name to enable global snapshot update mode.
     * Set {@code -Dsnapshot.update=true} to update all snapshots.
     */
    public static final String UPDATE_PROPERTY = "snapshot.update";

    /**
     * System property name to override the snapshot base directory.
     * Set {@code -Dsnapshot.baseDir=/path/to/dir} to store snapshots
     * under a specific directory instead of the auto-detected project root.
     *
     * <p>Example: {@code ./gradlew test -Dsnapshot.baseDir=/tmp/snap}</p>
     */
    ...
    /**
     * Gets the {@link ThymeleafRenderer} from the class-level store, creating it
     * on first access.
     */
    ...
    /**
     * Gets the {@link SnapshotManager} from the class-level store, creating it
     * on first access.
     */
    private SnapshotManager getOrCreateSnapshotManager(ExtensionContext context, ResolvedConfig config) {
        ExtensionContext.Store classStore = getClassStore(context);
        return classStore.getOrComputeIfAbsent(
                MANAGER_KEY,
                key -> new SnapshotManager(config.snapshotDir()),
