 ...
 *
 * <p>This error extends {@link AssertionError} so that it is recognized by JUnit as a test
 * assertion failure. The error message includes a unified diff between the expected (stored) and
 * actual (rendered) content for easy debugging.
 */
public final class SnapshotMismatchException extends AssertionError {
  ...
  private static String buildMessage(Path snapshotPath, String expected, String actual) {
    ...
    sb.append("\n\nTo update the snapshot, run with -D")
        .append(ThymeleafSnapshotExtension.UPDATE_PROPERTY)
        .append("=true");
   ...
   *
   * <p>The output follows the standard unified diff format with {@code @@ -L,C +L,C @@} hunk
   * headers. Lines prefixed with {@code -} are from the expected content, {@code +} from the actual
   * content, and a space for unchanged context lines.
   *
